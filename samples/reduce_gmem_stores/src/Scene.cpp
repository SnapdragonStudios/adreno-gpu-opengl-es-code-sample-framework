...
//--------------------------------------------------------------------------------------
VOID CSample::BeginFBO( FrameBufferObject* pFBO, BOOL bAttach)
{
    glBindFramebuffer( GL_FRAMEBUFFER, pFBO->m_hFrameBuffer );
    if ( bAttach )
    {
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D,
                               (pFBO)->m_hDepthTexture, 0);
...
VOID CSample::Render()
{
    ...
    if (m_bHasDS && m_bAttached == FALSE) {
        m_bAttached = TRUE;
    }
    else if (m_bHasDS == FALSE && m_bAttached == TRUE) {
        m_bAttached = FALSE;
    }
    ...
    // Render the scene into the offscreen FBO with a green background
    {
        ...
        BeginFBO( g_pOffscreenFBO0, m_bAttached);
    ...
    {
        BeginFBO( g_pOffscreenFBO1, m_bAttached);
        glClearColor( 0.0f, 0.5f, 0.0f, 1.0f );
        glClearDepthf(1.0);
        RenderScene( fTime );
    ...
    CHAR strMode[40];
    if (m_bHasDS)
        FrmSprintf( strMode, 40, "ColorDepthStencil Attachments");
    else
        FrmSprintf( strMode, 40, "Color Attachment Only");
